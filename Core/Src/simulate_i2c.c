...
void SimulateI2C_Init(SimulateI2C i2c)
{
     ...
     *  initStruct.Pin = i2c.SDAPin)|i2c.SCLPin);
  ...
  HAL_GPIO_WritePin(GPIOA, i2c1_SCL_Pin|i2c1_SDA_Pin|i2c2_SCL_Pin|i2c2_SDA_Pin, GPIO_PIN_SET);
	
  /*Configure GPIO pins : PAPin PAPin PAPin PAPin */
  GPIO_InitStruct.Pin = i2c1_SCL_Pin|i2c1_SDA_Pin|i2c2_SCL_Pin|i2c2_SDA_Pin;
...
uint8_t SimulateI2C_WaitAck(SimulateI2C i2c)
{
    ...
    HAL_GPIO_WritePin(i2c.SCLPort, i2c.SCLPin,GPIO_PIN_SET);
    delay_nus(I2C_DELAY_SCL_US);
    while(HAL_GPIO_ReadPin(i2c.SDAPort, i2c.SDAPin))
    {
        ucErrTime++;
        if(ucErrTime>250)
        {
        ...
        HAL_GPIO_ReadPin(i2c.SDAPort,i2c.SDAPin);
    }
    HAL_GPIO_WritePin(i2c.SCLPort, i2c.SCLPin,GPIO_PIN_RESET);
    return 0;
}
...
void SimulateI2C_SendByte(SimulateI2C i2c,uint8_t txd)
{
    ...
    for(t=0;t<8;t++)
    {
			HAL_GPIO_WritePin(i2c.SDAPort, i2c.SDAPin,((txd&0x80)>>7)==0?GPIO_PIN_RESET:GPIO_PIN_SET);
        txd<<=1;
        delay_nus(I2C_DELAY_SCL_US);
        HAL_GPIO_WritePin(i2c.SCLPort, i2c.SCLPin,GPIO_PIN_SET);
        delay_nus(I2C_DELAY_SCL_US);
...
uint8_t SimulateI2C_RecvByte(SimulateI2C i2c, unsigned char ack)
{
    ...
    for(i=0;i<8;i++ )
    {
        ...
        HAL_GPIO_WritePin(i2c.SCLPort, i2c.SCLPin,GPIO_PIN_SET);
        receive<<=1;
        if(HAL_GPIO_ReadPin(i2c.SDAPort, i2c.SDAPin))
            receive++;
        delay_nus(I2C_DELAY_SCL_US);
  }
    if (!ack)
